var issueId = Cookies.get("issueId");
...
function preLoad() {
    ...
    if (issueId != undefined || issueId != "") {
        ...
        alert('Issue is not defined! Please, search for project to complete request! ');
    }
}
...
$.when(preLoaded).done(function (data) {
    ...
    if (issueId == undefined || issueId == "") {
        ...
        $.ajax({
            ...
            success: function (issue) {
                ...
                $('#issue-info').append(
                    ...
                    '<tr><th>Project name:</th><td>' + issue.project.name + '</td></tr>' +
                    ...
                    '<tr><th>Creator:</th><td>' + issue.creator.fullName + '</td></tr>' +
                    '<tr><th>Assignee:</th><td>' + issue.assignee.fullName + '</td></tr>' +
        ...
        function connect() {
            var socket = new SockJS('/commenthandler');
        ...
        $('#btn-chat-send').on('click', function () {
            ...
            if (!message) {
                ...
                stompClient.send("/app/commenthandler", {}, JSON.stringify(
                    {
                        "comment": message,
                        "issueId": issueId,
                        "userId": $('#user-session-id').val(),
